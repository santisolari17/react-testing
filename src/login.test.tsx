...
describe('Login component tests', () => {
	...
	it('Renders correctly status label - invalid login', async () => {
		...
		const inputs = container.querySelectorAll('input');
		const loginButton = inputs[2];
		const asyncCheckLabelIsInDOM = () => expect(container.querySelector('label')).toBeInTheDocument();
		const asyncCheckLabelText = () => expect(container.querySelector('label')).toHaveTextContent('Login failed');

		fireEvent.click(loginButton);

		await waitFor(() => asyncCheckLabelIsInDOM())
		await waitFor(() => asyncCheckLabelText());
	});
	...
	it('Renders correctly status label - valid login', async () => {
		...
		const loginButton = inputs[2];
		const asyncCheckLabelIsInDOM = () => expect(container.querySelector('label')).toBeInTheDocument();
		const asyncCheckLabelText = () => expect(container.querySelector('label')).toHaveTextContent('Login successful');
